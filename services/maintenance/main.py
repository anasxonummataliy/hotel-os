...
class PriorityQueue:
    PRIORITY_ORDER = {PriorityLevel.CRITICAL: 4, PriorityLevel.HIGH: 3, PriorityLevel.NORMAL: 2, PriorityLevel.LOW: 1}
...
# ── Routes — fixed paths BEFORE parameterised ────────────────────────────────
...
async def report_issue(issue: MaintenanceIssueCreate, x_token: str = Header(...)):
    verify_token(x_token)
    if not db.get_room(issue.room_id):
        raise HTTPException(status_code=404, detail="Room not found")
    created = db.create_maintenance_issue({
        "room_id": issue.room_id, "description": issue.description,
        "priority": issue.priority, "status": "reported",
        "reported_by": issue.reported_by or "unknown", "resolved_at": None,
    })
...
async def get_room_issues(room_id: int, x_token: str = Header(...)):
    verify_token(x_token)
...
async def get_issue(issue_id: int, x_token: str = Header(...)):
    verify_token(x_token)
...
async def resolve_issue(issue_id: int, update: MaintenanceIssueUpdate, x_token: str = Header(...)):
    verify_token(x_token)
    issue = db.get_maintenance_issue(issue_id)
    if not issue:
        raise HTTPException(status_code=404, detail="Issue not found")
