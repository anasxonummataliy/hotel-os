...
export function Maintenance() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError('Failed to load maintenance data from backend.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleResolve = useCallback(async () => {
    ...
    setResolving(true);
    try {
      await resolveMaintenanceTicket(resolveTarget.id, resolveNotes || 'Resolved');
  ...
  const handleCreate = useCallback(async () => {
    if (!newForm.room_id || !newForm.description.trim()) {
      alert('Please select a room and enter a description.');
      return;
    }
    setSubmitting(true);
    try {
    ...
    <div style={{ padding: 24, fontFamily: 'Inter, sans-serif', flex: 1, overflowY: 'auto', display: 'flex', gap: 20, alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1, backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: '#1e293b', margin: 0 }}>Priority Queue</h3>
            <p style={{ fontSize: 11, color: '#94a3b8', margin: 0 }}>{openCount} open tickets — sorted by urgency (Critical first)</p>
          </div>
        ...
        {showNewForm && (
          <div style={{ margin: 14, padding: 16, backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10 }}>
            <h4 style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 700, color: '#1e293b' }}>Report New Issue</h4>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 120px' }}>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>Room</label>
              ...
              <div style={{ flex: '2 1 200px' }}>
                ...
                <input style={inputStyle} placeholder="Describe the issue…" value={newForm.description} onChange={e => setNewForm(f => ({ ...f, description: e.target.value }))} />
              </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: '#f8fafc' }}>
                {['#', 'Room', 'Description', 'Urgency', 'Status', 'Reported By', 'Action'].map(h => (
              ...
              {sorted.length === 0 && (
                <tr>
                  <td colSpan={7} style={{ padding: 32, textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>
                    No maintenance tickets — all clear!
                  </td>
                </tr>
              )}
              {sorted.map((ticket, i) => {
                  ...
                  <tr key={ticket.id} style={{ borderTop: '1px solid #f1f5f9', opacity: isResolved ? 0.55 : 1, backgroundColor: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                    ...
                    <td style={{ padding: '12px 14px', maxWidth: 240 }}>
                      ...
                      {ticket.resolution_notes && (
                        <p style={{ fontSize: 11, color: '#94a3b8', margin: '2px 0 0' }}>Fix: {ticket.resolution_notes}</p>
                      )}
                    </td>
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      <span style={{ fontSize: 12, color: '#64748b', textTransform: 'capitalize' }}>{ticket.status}</span>
                    </td>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#fff', borderRadius: 12, width: 440, overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#1e293b' }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: '#f8fafc', margin: 0 }}>Log Resolution — #{resolveTarget.id}</h3>
            ...
            <div style={{ padding: 20 }}>
              <div style={{ backgroundColor: '#f8fafc', borderRadius: 8, padding: '12px 14px', marginBottom: 14 }}>
                <p style={{ fontSize: 13, fontWeight: 600, color: '#1e293b', margin: '0 0 4px' }}>
                  Room {rooms.find(r => r.id === resolveTarget.room_id)?.number ?? resolveTarget.room_id}
                </p>
                <p style={{ fontSize: 12, color: '#64748b', margin: 0 }}>{resolveTarget.description}</p>
              </div>
              <label style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 6 }}>Resolution Notes</label>
              <textarea
                rows={4}
                placeholder="Describe the fix applied, parts used, etc..."
                value={resolveNotes}
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
                <button onClick={() => setResolveTarget(null)} style={{ flex: 1, padding: 10, borderRadius: 8, border: '1px solid #e2e8f0', backgroundColor: '#fff', fontSize: 13, cursor: 'pointer', color: '#64748b' }}>Cancel</button>
                <button onClick={handleResolve} disabled={resolving} style={{ flex: 2, padding: 10, borderRadius: 8, border: 'none', backgroundColor: '#16a34a', color: '#fff', fontSize: 13, fontWeight: 700, cursor: resolving ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  <CheckCircle size={14} /> {resolving ? 'Saving…' : 'Mark as Resolved'}
                </button>
              </div>
            </div>
          </div>
