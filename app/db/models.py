...
class User(Base):
    ...
    guest: Mapped[Optional["Guest"]] = relationship(
        "Guest", back_populates="user", foreign_keys=[guest_id]
    )
...
class Room(Base):
    ...
    last_cleaned: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, default=func.now()
    )
    current_guest_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("guests.id"), nullable=True
    )

    # relationships
    bookings: Mapped[List["Booking"]] = relationship(
        "Booking", back_populates="room", foreign_keys="Booking.room_id"
    )
    orders: Mapped[List["Order"]] = relationship("Order", back_populates="room")
    maint_issues: Mapped[List["MaintenanceIssue"]] = relationship(
        "MaintenanceIssue", back_populates="room"
    )
...
class Guest(Base):
    ...
    user: Mapped[Optional["User"]] = relationship(
        "User", back_populates="guest", foreign_keys="User.guest_id"
    )
...
class Booking(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(30), nullable=False, default="checked_in"
    )
    ...
    guest: Mapped["Guest"] = relationship(
        "Guest", back_populates="bookings", foreign_keys=[guest_id]
    )
    room: Mapped["Room"] = relationship(
        "Room", back_populates="bookings", foreign_keys=[room_id]
    )
